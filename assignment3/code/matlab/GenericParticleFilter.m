...
%    pf = GenericParticleFilter(veh, sensor, Q, L, 1000);
...
% std should be a weighted std (x-mean)' W (x-mean)  ???
    ...
    methods
        function pf = GenericParticleFilter(robot, sensor, Q, L, np, varargin)
            %GenericParticleFilter.GenericParticleFilter Particle filter constructor
            %
            % PF = GenericParticleFilter(VEHICLE, SENSOR, Q, L, NP, OPTIONS) is a particle
            ...
            %
            % Notes::
            % - GenericParticleFilter subclasses Handle, so it is a reference object.
        ...
        function init(pf, opt)
            %GenericParticleFilter.init Initialize the particle filter
            %
        ...
        function run(pf, niter, varargin)
            %GenericParticleFilter.run Run the particle filter
            %
        ...
        function plot_pdf(pf)
            %GenericParticleFilter.plot_pdf Plot particles as a PDF
            %
        ...
        function plot_xy(pf, varargin)
            %GenericParticleFilter.plot_xy Plot vehicle position
            %
        ...
        function display(pf)
            %GenericParticleFilter.display Display status of particle filter object
            %
            % PF.display() displays the state of the GenericParticleFilter object in
            % human-readable form.
            %
            % Notes::
            % - This method is invoked implicitly at the command line when the result
            %   of an expression is a GenericParticleFilter object and the command has no trailing
            %   semicolon.
            %
            % See also GenericParticleFilter.char.
        ...
        function s = char(pf)
            %GenericParticleFilter.char Convert to string
            %
            % PF.char() is a string representing the state of the GenericParticleFilter
            % object in human-readable form.
            %
            % See also GenericParticleFilter.display.
            s = sprintf('GenericParticleFilter object: %d particles', pf.nparticles);
            if ~isempty(pf.robot)
